...
import com.whileloop.instareels.persistance.Document
import com.whileloop.instareels.utils.loadListImage
import com.whileloop.instareels.viewmodels.HomeViewModel
...
class Home : Fragment() {
    ...
    var allReels: LiveData<List<Document?>?>? = null
    ...
    private fun setLastPost() {
        ...
        allReels?.observe(requireActivity(), { documents: List<Document?>? ->
            if (documents!!.isNotEmpty()) {
                lastPost.visibility = View.VISIBLE
                val lastPost = documents[0]!!
                dp.loadListImage(lastPost.dpUrl)

                listImage.loadListImage(lastPost.videoUri)
                caption.text = lastPost.caption
                handle.text = lastPost.handle

                if (lastPost.isVideo == "false") {
